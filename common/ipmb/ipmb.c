	...
	while (1) {
		...
		if (IS_RESPONSE(current_msg_tx->buffer)) { // Send a response message
			...
			if (ipmb_cfg.interface == I2C_IF) {
				...
				printf("[%s] Not support interface(%d) for index(%d)\n", __func__,
...
void IPMB_RXTask(void *pvParameters, void *arvg0, void *arvg1)
{
	...
	while (1) {
		...
		if (rx_len > 0) {
			if (DEBUG_IPMB) {
				printf("Recieve an IPMB message from bus(%d) data[%d](",
				       ipmb_cfg.bus, rx_len);
				for (i = 0; i < rx_len; i++) {
			...
			if (IS_RESPONSE(current_msg_rx->buffer)) { // Response message
				...
				if (IPMB_config_table[ipmb_cfg.index].channel == ME_IPMB &&
					...
					if (DEBUG_IPMB) {
						printf("[%s] Recieve a req message, netfn(0x%x) InfS(0x%x) seq_s(%d)\n",
...
ipmb_error ipmb_send_request(ipmi_msg *req, uint8_t index)
{
	...
	if (ret) {
		printf("[%s] Failed to allocates memory\n", __func__);
...
static void register_slave_dev(void)
{
#ifdef DEV_IPMB_0
...
void create_ipmb_threads(uint8_t index)
{
	...
	if (k_mutex_init(&mutex_id[index])) {
		printf("Failed to create threads,Tx(%s) Rx(%s)\n",
		       IPMB_config_table[index].tx_thread_name,
		       IPMB_config_table[index].rx_thread_name);
		return;
	}
...
void ipmb_init(void)
{
	...
	if (k_mutex_init(&mutex_send_req)) {
		printf("Failed to initial IPMB send request mutex\n");
	}
	if (k_mutex_init(&mutex_send_res)) {
		printf("Failed to initial IPMB send response mutex\n");
	}
	if (k_mutex_init(&mutex_read)) {
		printf("Failed to initial IPMB read mutex\n");
	}
	...
	// Create IPMB sequence number timout thread

...
static uint8_t data_buf[I3C_MAX_XFER_NUM][I3C_MAX_BUFF_SIZE];
...
int i3c_priv_xfer(I3C_MSG *msg, uint8_t retry)
{
	...
	for (int i = 0; i < I3C_MAX_XFER_NUM; i++) {
		if (!i) {
			/* Data for write */
			msg->xfer[i].rnw = 0;
			msg->xfer[i].data.out = data_buf[i];
			msg->xfer[i].len = msg->tx_len;
			memcpy(&data_buf[0], &msg->data[0], msg->tx_len);
		} else {
			/* Data for read */
			msg->xfer[i].rnw = 1;
			msg->xfer[i].data.in = data_buf[i];
...
int i3c_smq_read() //(I3C_MSG *msg)
{
	...
	int ret = 0;
	printf("[%s] starting slave read\n", __func__);
	i3c_slave_mqueue_read(dev_i3c[msg->bus], data_buf[0], msg->rx_len);
	printf("[%s] complete slave read\n", __func__);
	printf("[%s] starting data_buf print\n", __func__);	
	for (int i = 0; i < I3C_MAX_XFER_NUM; ++i) {
		for (int j = 0; j < I3C_MAX_BUFF_SIZE; ++j) {
			printf("0x%02x ", data_buf[i][j]);
		}
		printf("\n");
	}
	printf("[%s] complete data_buf print\n", __func__);	
	return ret;
}
...
int i3c_smq_write(I3C_MSG *msg)
{
	...
	if (!dev_i3c[msg->bus]) {
		printf("[%s] bus%u not defined\n", __func__, msg->bus);
		return -ENODEV;
	}

	if (I3C_DEBUG) {
		LOG_HEXDUMP_DBG(msg->data, msg->tx_len, "I3C_SMQ_WRITE");
	}

	ret = k_mutex_lock(&i3c_mutex[msg->bus], K_SECONDS(1));
	if (ret) {
		printf("[%s] k_mutex bus%u lock failed, ret: %d\n", __func__, msg->bus, ret);
		return ret;
	}

	/* TODO: if tx_len > data_buf size */

	memcpy(&data_buf[0], &msg->data[0], msg->tx_len);
	ret = i3c_slave_mqueue_write(dev_i3c[msg->bus], data_buf[0], msg->tx_len);
	k_mutex_unlock(&i3c_mutex[msg->bus]);
	printf("[%s] smq write sucessfully\n", __func__);
	return ret;
}

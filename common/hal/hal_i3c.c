...
int i3c_smq_write(I3C_MSG *msg)
{
	...
	for (int i = 0; i < retry; i++) {
		...
		if (ret == -EBUSY) {
			//LOG_HEXDUMP_ERR(msg->data, msg->tx_len, "data");
			//LOG_ERR ("retry = %d", i);
			k_usleep(10 * 1000);
		} else {
			break;
	...
	if (ret == -EBUSY )
	{
		//LOG_ERR ("!!!!!!! i3c_slave_mqueue_write fail: ret = %d !!!!!",ret);
	}
	//k_usleep(1 * 1000);
	k_mutex_unlock(&mutex_write[msg->bus]);
	return ret;
...
void util_init_i3c(void)
{
	...
	for (int i = 0; i <= I3C_MAX_NUM; ++i) {
		if (k_mutex_init(&mutex_read[i]) == 0) {
			LOG_ERR("Muxex %d init error.",i);
		}
		if (k_mutex_init(&mutex_write[i]) == 0) {
			LOG_ERR("Muxex %d init error.",i);
		}
	}
}

...
static bool table_access_check(uint8_t table_idx)
{
...
static sensor_cfg *sensor_get_idx_by_sensor_num(uint8_t table_idx, uint16_t sensor_num)
{
	if (table_idx < sensor_monitor_count) {
...
static int sensor_access(const struct shell *shell, uint8_t table_idx, sensor_cfg *cfg,

...
void pldm_sensor_polling_handler(void *arug0, void *arug1, void *arug2)
{
	...
	for (sensor_num = 0; sensor_num < pldm_sensor_count; sensor_num++) {
		if (pldm_sensor_list[thread_id][sensor_num].pldm_sensor_cfg.pre_sensor_read_hook) {
			if (!pldm_sensor_list[thread_id][sensor_num]
				     .pldm_sensor_cfg.pre_sensor_read_hook(
		...
		if (ret < 0) {
			pldm_sensor_list[thread_id][sensor_num].pldm_sensor_cfg.cache_status =
				PLDM_SENSOR_FAILED;
			LOG_ERR("Failed to init sensor 0x%x",
				pldm_sensor_list[thread_id][sensor_num]
		...
		if (pldm_sensor_list[thread_id][sensor_num].pldm_sensor_cfg.post_sensor_read_hook) {
			if (!pldm_sensor_list[thread_id][sensor_num]
				     .pldm_sensor_cfg.post_sensor_read_hook(
					     &pldm_sensor_list[thread_id][sensor_num]
						      .pldm_sensor_cfg,
					     pldm_sensor_list[thread_id][sensor_num]
						     .pldm_sensor_cfg.post_sensor_read_args,
					     NULL)) {
				pldm_sensor_list[thread_id][sensor_num]
					.pldm_sensor_cfg.cache_status = PLDM_SENSOR_FAILED;
				continue;
	...
	while (1) {
		for (sensor_num = 0; sensor_num < pldm_sensor_count; sensor_num++) {
			...
			if (!pldm_sensor_is_interval_ready(&pldm_sensor_list[thread_id][sensor_num])) {
				continue;
			}
		...
		LOG_INF("sensor0x%x, value0x%x, status 0x%x",
			pldm_sensor_list[thread_id][sensor_num].pdr_numeric_sensor.sensor_id,
			pldm_sensor_list[thread_id][sensor_num].pldm_sensor_cfg.cache,
			pldm_sensor_list[thread_id][sensor_num].pldm_sensor_cfg.cache_status);

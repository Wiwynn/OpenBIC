...
int pdr_init(void)
{
	...
	uint32_t record_handle = 0x0, largest_record_size = 0;
	...
	if (pdr_count != 0) {
		...
		if (numeric_sensor_table == NULL) {
			LOG_ERR("Failed to load numeric sensor PDR table");
			return -1;
		}
	...
	if (pdr_count != 0) {
		total_record_count += pdr_count;
		sensor_auxiliary_names_table = (PDR_sensor_auxiliary_names *)malloc(pdr_count * sizeof(PDR_sensor_auxiliary_names));
		plat_load_aux_sensor_names_pdr_table(sensor_auxiliary_names_table);
		if (sensor_auxiliary_names_table == NULL) {
			LOG_ERR("Failed to load sensor auxiliary names PDR table");
			return -1;
		}
		pdr_info->repository_size += pdr_count * sizeof(PDR_sensor_auxiliary_names);

		for (uint32_t i = 0; i < pdr_count; i++) {
			sensor_auxiliary_names_table[i].pdr_common_header.record_handle = record_handle;
			sensor_auxiliary_names_table[i].pdr_common_header.data_length +=
				(sizeof(PDR_sensor_auxiliary_names) - sizeof(PDR_common_header));

			// Modify sensor name to UTF16-BE
			for (int j = 0; sensor_auxiliary_names_table[i].sensorName[j] != 0x0000; j++) {
        		sensor_auxiliary_names_table[i].sensorName[j] = sys_cpu_to_be16(sensor_auxiliary_names_table[i].sensorName[j]);
			}
...
int get_pdr_table_via_record_handle(uint8_t *record_data, uint32_t record_handle)
{
	...
	if (record_handle < numeric_sensor_pdr_count) {
		for (int i = 0; i < numeric_sensor_pdr_count; i++) {
			if (numeric_sensor_table[i].pdr_common_header.record_handle == record_handle) {
				memcpy(record_data, &numeric_sensor_table[i], sizeof(PDR_numeric_sensor));
		...
		for (int i = 0; i < aux_sensor_name_pdr_count; i++) {
			if (sensor_auxiliary_names_table[i].pdr_common_header.record_handle == record_handle) {
				memcpy(record_data, &sensor_auxiliary_names_table[i], sizeof(PDR_sensor_auxiliary_names));
...
__weak uint16_t plat_get_pdr_size(uint8_t pdr_type)
{

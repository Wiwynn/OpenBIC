			     ...
			     uint8_t *mctp_data, uint32_t mctp_data_len, mctp_ext_params extra_data)
{
	...
	i3c_hdr *hdr = (i3c_hdr *)send_buf;
	hdr->cmd_code = MCTP_I3C_CMD_CODE;
	/* bit 0 always set */
	hdr->src_addr = mctp_inst->medium_conf.i3c_conf.addr + 1;
	/* extend 1 byte src addr */
	hdr->byte_count = 1 + mctp_data_len;
	memcpy(send_buf + sizeof(*hdr), mctp_data, mctp_data_len);

	uint8_t pec = 0;
	if (cal_pec(extra_data.i3c_ext_params.addr, send_buf, send_len, &pec) == MCTP_ERROR)
		return MCTP_ERROR;
				...
				mctp_ext_params *extra_data)
{
	...
	/* TODO: read data from i3c */
	uint8_t ret = 0;
	ret = i2c_target_read(mctp_inst->medium_conf.i3c_conf.bus, rdata, 256, &rlen);
	if (ret) {
		LOG_ERR("i2c_target_read fail, ret %d\n", ret);
		return 0;
	}

	i3c_hdr *hdr = (i3c_hdr *)rdata;
...
*/
	...
	if (is_pec_exist) {
		if (is_pec_vaild(mctp_inst->medium_conf.i3c_conf.addr, rdata, rlen) == false)
			return 0;
	}
	...
	memcpy(buf, rdata + sizeof(i3c_hdr), rt_size);
				 ...
				 mctp_ext_params extra_data)
{
	...
	LOG_HEXDUMP_DBG(buf, len, "mctp_i3c_write receive data");
	...
	LOG_DBG("i3c_ext_params addr = %x", extra_data.i3c_ext_params.addr);
	LOG_HEXDUMP_DBG(send_buf, send_len, "mctp_i3c_write make header");

	/* TODO: write data to i3c */
	int status;
	I3C_MSG i3c_msg;
	i3c_msg.bus = mctp_inst->medium_conf.i3c_conf.bus;
	i3c_msg.target_addr = extra_data.i3c_ext_params.addr >> 1;
	i3c_msg.tx_len = send_len;
	memcpy(&i3c_msg.data[0], send_buf, send_len);
	status = i3c_smq_write(&i3c_msg);
	if (status)
...
uint8_t mctp_i3c_init(mctp *mctp_inst, mctp_medium_conf medium_conf)
{
	...
	mctp_inst->read_data = mctp_i3c_read;
	mctp_inst->write_data = mctp_i3c_write;

...
*/
	...
	rc = handler(mctp_inst, buf + sizeof(*hdr), len - sizeof(*hdr), (hdr->req_d_id)&0x1F, resp_buf + sizeof(*hdr),
		     &resp_len, &ext_params);
...
int pldm_send_ipmi_response(uint8_t interface, ipmi_msg *msg)
{
	...
	printk("%s Wiwynn Debug %d: inst 0x%x\n", __func__, __LINE__, pmsg.hdr.inst_id);
...
int pldm_send_ipmi_request(ipmi_msg *msg)
{
	...
	uint8_t res_len = mctp_pldm_read(pal_get_mctp(medium_type, target), &pmsg, rbuf, sizeof(rbuf));

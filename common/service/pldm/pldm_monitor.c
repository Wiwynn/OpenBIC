				...
				uint8_t *resp, uint16_t *resp_len, void *ext_params)
{
	...
	status = pldm_sensor_get_reading_from_cache(sensor_number, &reading, &sensor_operational_state);
				     ...
				     void *ext_params)
{
	...
	response_count = get_pdr_table_via_record_handle(&res_p->record_data[0], req_p->record_handle);
	if (response_count <= 0) {

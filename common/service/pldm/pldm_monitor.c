...
__weak uint8_t plat_pldm_set_state_effecter_state_handler(const uint8_t *buf, uint16_t len,
							  uint8_t *resp, uint16_t *resp_len, struct pldm_state_effecter_info *info_p)
{
	CHECK_NULL_ARG_WITH_RETURN(buf, PLDM_ERROR);
	CHECK_NULL_ARG_WITH_RETURN(resp, PLDM_ERROR);
	CHECK_NULL_ARG_WITH_RETURN(resp_len, PLDM_ERROR);
...
void pldm_load_state_effecter_table(uint16_t state_effecter_index)
{
	...
	state_effecter_table = malloc(state_effecter_index * sizeof(struct pldm_state_effecter_info));

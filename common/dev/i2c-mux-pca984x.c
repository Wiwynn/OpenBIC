 ...
 * 
 * Set 5 seconds timeout if mutex lock failed.
 ...
 * @param msg->lock->lock_count member of k_mutex, 
 * 
 * @retval true  mutex lock and switch channel successfully.
...
/*bool i2c_mux_pca9846_lock(I2C_MSG *msg)
{
	...
	if (k_mutex_lock(msg->lock, K_MSEC(5000))) {
		rintf("[%s] Failed to lock bus%u mux0x%x. Force re-init the mutex\n",
		       __func__, msg->bus, msg->target_addr);
		k_mutex_init(msg->lock);
	}

	printf("[%s] msg->lock->owner: %p\n", __func__, msg->lock->owner);
	printf("[%s] msg->lock->lock_count: %u\n", __func__, msg->lock->lock_count);

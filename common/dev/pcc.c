...
static void process_postcode(void *arvg0, void *arvg1, void *arvg2)
{
	...
	while (1) {
		...
		uint8_t *msg2 = (uint8_t *)malloc(4 * sizeof(uint8_t));
		...
		for (; send_index != current_read_index;
		    ...
		    // uint8_t *msg2 = (uint8_t *)malloc(sizeof(4));
			msg2[0] = pcc_read_buffer[send_index] & 0xFF;
			msg2[1] = (pcc_read_buffer[send_index] >> 8) & 0xFF;
			msg2[2] = (pcc_read_buffer[send_index] >> 16) & 0xFF;
			msg2[3] = (pcc_read_buffer[send_index] >> 24) & 0xFF;

			// pal_send_post_code_to_bmc(msg2, 4);

			LOG_ERR("Post code data2 = %d %d %d %d", msg2[0], msg2[1], msg2[2], msg2[3]);

			uint8_t t[3] = { msg2[0], msg2[1], msg2[2] };
			pal_send_post_code_to_bmc(t, 3);

			// LOG_ERR("Post code data = %d %d %d %d", msg->data[4], msg->data[5], msg->data[6], msg->data[7]);

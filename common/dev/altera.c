...
int get_register_via_i2c(int reg, int *val)
{
	...
	i2c_msg.slave_addr = altera_max10_config.slave_addr;
...
int cpld_altera_max10_fw_update(uint32_t offset, uint16_t msg_len, uint8_t *msg)
{
	...
	buffer_offset = 0;
	for (addr = (altera_max10_config.update_start_addr + offset); addr < (altera_max10_config.update_start_addr + offset + msg_len);
	     addr += 4) {
		// Get 4 bytes

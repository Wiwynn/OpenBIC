...
uint8_t nvme_read(void *arg, int *reading)
{
	...
	switch (cfg->offset) {
	case NVME_TEMP_OFFSET: {
		if (nvme_cal_pec(cfg->target_addr, cfg->offset, read_resp, rx_len - 1, &pec) != 0) {
			LOG_ERR("sensor_num 0x%02x cal nvme pec fail!", cfg->num);
			return SENSOR_UNSPECIFIED_ERROR;
		}

		if (read_resp[NVMe_PEC_INDEX] != pec) {
			LOG_ERR("sensor_num 0x%02x check nvme pec error! (%02x/%02x)", cfg->num,
				read_resp[NVMe_PEC_INDEX], pec);

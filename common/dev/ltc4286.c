...
typedef union {
	...
	struct {
		volatile uint16_t vpwr_select : 1;
		volatile uint16_t vrange_select : 1;
		volatile uint16_t reserved_1 : 8; // bit[9:2] are reserved.
		volatile uint16_t ilim : 4;
		volatile uint16_t reserved_2 : 2; // bit[15:14] are reserved.
	} fields;
} mfr_config_1_t;

mfr_config_1_t mfr_config_1;
...
uint8_t ltc4286_read(uint8_t sensor_num, int *reading)
{
	...
	for (int cnt = 0; cnt < ARRAY_SIZE(ltc4286_mbr_table); cnt++) {
		if (type == ltc4286_mbr_table[cnt].type)
		{
			if ((ltc4286_mbr_table[cnt].voltage_range == BOTH) || 
				(ltc4286_mbr_table[cnt].voltage_range == mfr_config_1.fields.vrange_select))
			{
				index = cnt;
			}
			else
	...
	val = (1 / M) * ((msg.data[1] << 8) | msg.data[0]) * ltc4286_mbr_table[index].R - ltc4286_mbr_table[index].B;
...
uint8_t ltc4286_init(uint8_t sensor_num)
{
	...
	data[1] = init_args->mfr_config_1 & 0xFF;
	data[2] = init_args->mfr_config_1 >> 8 & 0xFF;
...
init_param:
	...
	if (msg.data[0] == LTC4286_MFR_CONFIG1) {
		mfr_config_1.value = (msg.data[1] | (msg.data[2] << 8));
	}

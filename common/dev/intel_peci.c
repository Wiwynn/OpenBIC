...
static bool read_cpu_power(uint8_t addr, int *reading)
{
	...
	if (complete_code == PECI_CC_RSP_TIMEOUT ||
		...
		if (!is_retry_success) {
			// printf("Failed to send PECI command(0x%x), index(0x%x), parameter(0x%x)\n", PECI_RD_PKG_CFG0_CMD);
			free(readbuf);
			return false;
		}
	}
...
static bool get_cpu_tjmax(uint8_t addr, int *reading)
{
	...
	if (peci_retry_read(PECI_CMD_RD_PKG_CFG0, addr, RDPKG_IDX_TJMAX_TEMP, param, rlen, rbuf) ==
	    false)
		return false;
...
static bool get_cpu_margin(uint8_t addr, int *reading)
{
	...
	if (peci_retry_read(PECI_CMD_RD_PKG_CFG0, addr, RDPKG_IDX_PKG_TEMP, param, rlen, rbuf) ==
	    false)
		return false;
...
uint8_t intel_peci_read(uint8_t sensor_num, int *reading)
{
	...
	case PECI_TEMP_CPU:
		...
		break;
	case PECI_PWR_CPU:
		ret_val = get_cpu_pwr(cfg->slave_addr, reading);
		break;
	default:
		break;
	}

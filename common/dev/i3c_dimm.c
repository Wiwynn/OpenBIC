...
uint8_t i3c_dimm_read(uint8_t sensor_num, int *reading)
{
	...
	case DIMM_PMIC_SWA_PWR:
		ret = pal_get_pmic_pwr(sensor_num, (uint8_t *)&data);
		if (ret != 0) {
		...
		break;
	case DIMM_SPD_TEMP:
		ret = pal_get_spd_temp(sensor_num, (uint8_t *)&data);
		if (ret != 0) {
			return SENSOR_FAIL_TO_ACCESS;
		}

		/* Accround SPEC 3.2.2 Thermal Sensor Registers Read Out Mechanism (p.105)

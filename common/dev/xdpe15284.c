...
bool xdpe15284_enable_write_protect(uint8_t bus, uint8_t addr)
{
	if (write_protect_default_val != XDPE15284_DISABLE_WRITE_PROTECT_VAL) {
		...
		return xdpe15284_set_write_protect_reg(bus, addr, XDPE15284_ENABLE_WRITE_PROTECT);
	}
...
bool xdpe15284_disable_write_protect(uint8_t bus, uint8_t addr)
{
	if (write_protect_default_val != XDPE15284_DISABLE_WRITE_PROTECT_VAL) {
		...
		return xdpe15284_set_write_protect_reg(bus, addr, XDPE15284_DISABLE_WRITE_PROTECT);
	}
...
bool xdpe15284_get_checksum(uint8_t bus, uint8_t addr, uint8_t *checksum)
{
	...
	bool ret = xdpe15284_disable_write_protect(bus, addr);
	if (ret != true) {
	...
	ret = xdpe15284_enable_write_protect(bus, addr);
	if (ret != true) {

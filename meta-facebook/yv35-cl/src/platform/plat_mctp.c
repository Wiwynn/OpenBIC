...
{
	LOG_INF("initializing mctp");

	/** init mctp for bmc bus **/
	mctp *mctp_instance = mctp_init();
	mctp_set_medium_configure(mctp_instance, MCTP_MEDIUM_TYPE_I3C, mctp_instance->medium_conf);
	mctp_reg_msg_rx_func(mctp_instance, mctp_msg_recv);
	mctp_instance->medium_conf.i3c_conf.bus = I3C_BUS_BMC;
	// mctp_instance->medium_conf.smbus_conf.addr = I3C_ADDR_BIC;

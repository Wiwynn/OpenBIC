...
//		msg.timeout_cb_fn_args = p;
//
//		mctp_pldm_send_msg(find_mctp_by_i3c(p->bus), &msg);
//	}
//}
...
void plat_mctp_init(void)
{
	...
	for (uint8_t i = 0; i < ARRAY_SIZE(i3c_port); i++) {
		...
		//LOG_INF("p->mctp_inst mctp_init (allocate memory) complete");
...
void plat_mctp_test(void)
{
	for (int i = 0; i < 3; ++i) {
		k_msleep(60000);

		mctp *mctp_inst = find_mctp_by_i3c(0);
		mctp_inst->medium_conf.i3c_conf.addr = I3C_STATIC_ADDR_BMC;
		
		I3C_MSG *i3c_msg = (I3C_MSG *)malloc(sizeof(I3C_MSG));
		i3c_msg->tx_len = 10;
		i3c_msg->data[0] = 0x01;			// MCTP header version
		i3c_msg->data[1] = MCTP_EID_BMC;	// Destination EID
		i3c_msg->data[2] = MCTP_EID_SELF;	// Source EID
		i3c_msg->data[3] = 0xC0;			// Flags
		i3c_msg->data[4] = 0x01;			// bit7: Integrity Check bit, bit[6:0]: Message Type, PLDM
		i3c_msg->data[5] = 0x80;			// PLDM data[1], 10000000
		i3c_msg->data[6] = 0x3F;			// PLDM data[2], Header+PLDM type (OEM 0x3F)
		i3c_msg->data[7] = 0x01;			// PLDM data[3], PLDM command code 0x01 = IPMI
		i3c_msg->data[8] = 0x18;			// IPMI NetFn
		i3c_msg->data[9] = 0x04;			// IPMI CMD

		mctp_ext_params ext_params;
		ext_params.type = MCTP_MEDIUM_TYPE_I3C;

		mctp_bridge_msg(mctp_inst, &i3c_msg->data[0], i3c_msg->tx_len, ext_params);
	}
}

...
void pal_fix_sensor_config()
{
	...
	if (get_bic_class() == SYS_CLASS_1) {
		...
		case SYS_BOARD_EVT2:
			...
			while (sensor_num) {
				add_sensor_config(adm1278_sensor_config_table[sensor_num - 1]);
				sensor_num--;
			}
			break;
		case SYS_BOARD_EVT3_EFUSE:
			...
			while (sensor_num) {
				if (get_2ou_status()) {
					 ...
					 */
					mp5990_sensor_config_table[sensor_num - 1].init_args =
						&mp5990_init_args[1];
				...
				add_sensor_config(mp5990_sensor_config_table[sensor_num - 1]);
				sensor_num--;
			}
			break;
		case SYS_BOARD_EVT3_HOTSWAP:
			...
			if ((voltage_hsc_type_adc > 0.5 - (0.5 * 0.15)) &&
				...
				while (sensor_num) {
					add_sensor_config(
						adm1278_sensor_config_table[sensor_num - 1]);
					sensor_num--;
				}
				...
				while (sensor_num) {
					add_sensor_config(
						ltc4286_sensor_config_table[sensor_num - 1]);
					sensor_num--;
				}
			} else {

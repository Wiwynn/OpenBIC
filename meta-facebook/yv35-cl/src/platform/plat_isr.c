...
void ISR_DC_ON()
{
	...
	if (get_DC_status() == true) {
		...
		if ((gpio_get(FM_SLPS3_PLD_N) == GPIO_HIGH) && (gpio_get(RST_RSMRST_BMC_N) == GPIO_HIGH)) {
			addsel_msg_t sel_msg;
...
void ISR_HSC_THROTTLE()
{
	...
	if (gpio_get(RST_RSMRST_BMC_N) == GPIO_HIGH) {
		if ((gpio_get(PWRGD_SYS_PWROK) == GPIO_LOW) && (get_DC_off_delayed_status() == false)) {
			return;
		} else {
			if ((gpio_get(IRQ_SML1_PMBUS_ALERT_N) == GPIO_HIGH) && (is_hsc_throttle_assert == true)) {
				sel_msg.event_type = IPMI_OEM_EVENT_TYPE_DEASSART;
				is_hsc_throttle_assert = false;
			} else if ((gpio_get(IRQ_SML1_PMBUS_ALERT_N) == GPIO_LOW) && (is_hsc_throttle_assert == false)) {
				sel_msg.event_type = IPMI_EVENT_TYPE_SENSOR_SPEC;
...
void ISR_SOC_THMALTRIP()
{
	...
	if (gpio_get(RST_PLTRST_PLD_N) == GPIO_HIGH) {
		if (gpio_get(H_CPU_MEMTRIP_LVC3_N) == GPIO_HIGH) { // Reference pin for memory thermal trip event
...
void ISR_PCH_THMALTRIP()
{
	...
	if (gpio_get(FM_PCHHOT_N) == GPIO_LOW) {
		if ((gpio_get(RST_PLTRST_PLD_N) == GPIO_HIGH) && get_post_status() && (is_pch_assert == false)) {
			sel_msg.event_type = IPMI_EVENT_TYPE_SENSOR_SPEC;
			is_pch_assert = true;
		}

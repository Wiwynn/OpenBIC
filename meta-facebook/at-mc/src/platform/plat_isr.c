...
bool cxl_set_eid_work_handler(uint8_t cxl_card_id)
{
	...
	if (!cxl_work_item[cxl_card_id].is_device_reset) {
		return false;
	}
	...
	meb_mux.channel = cxl_work_item[cxl_card_id].cxl_channel;
	...
	/** Set endpoint id **/
	set_cxl_endpoint(MCTP_EID_CXL, cxl_card_id);
...
void cxl_ioexp_alert_handler(struct k_work *work_item)
{
	...
	cxl_info->is_interrupt_ongoing = true;
	...
	/** Enable mux channel **/
	ret = set_mux_channel(meb_mux, false);
	if (ret == false) {
		return;
	}

	ret = set_mux_channel(cxl_mux, false);
	if (ret == false) {
		return;
	}
	...
	/** Set mux channel to 0, let previous thread retry **/
	meb_mux.channel = 0;
	...
	cxl_info->is_interrupt_ongoing = false;
}

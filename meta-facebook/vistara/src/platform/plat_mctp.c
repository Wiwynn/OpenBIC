...
static void set_endpoint_resp_timeout(void *args)
{
	...
	LOG_WRN("=====Endpoint 0x%x set endpoint failed on bus %d \n", p->endpoint,
	       p->bus);
}
...
void plat_mctp_init()
{
	...
	for (uint8_t i = 0; i < ARRAY_SIZE(smbus_port); i++) {
		...
		LOG_WRN("=====bus = %x, addr = %x", p->conf.smbus_conf.bus, p->conf.smbus_conf.addr);
		...
		LOG_WRN("=====mctp_set_medium_configure %s",
	...
	if (gpio_get(FM_BIOS_POST_CMPLT_BIC_N) == GPIO_LOW) {

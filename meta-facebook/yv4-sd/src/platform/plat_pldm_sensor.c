...
const prochot_sensor_info prochot_sensor_table[] = {
	// { sensor_id, event_bit }
...
{
	size_t num_sensors = sizeof(prochot_sensor_table) / sizeof(prochot_sensor_table[0]);
	...
	while (1) {
		...
		for (size_t i = 0; i < num_sensors; i++) {
			...
			if (pldm_sensor_get_info_via_sensor_id(sensor_id, &resolution, &offset,
				...
				event_bit |=
					(event_bit_history & prochot_sensor_table[i].event_bit);
				continue;
			}

...
pldm_sensor_info mb_temp_pldm_sensor_table[] = {
	{
		{
			// Inlet Temperature
			/*** PDR common header***/
...
pldm_sensor_info *plat_load_pldm_sensor(int thread_id)
{
	...
	case MB_TEMP_SENSOR_THREAD_ID:
		return mb_temp_pldm_sensor_table;
	default:
...
int plat_get_pldm_sensor_count(int thread_id)
{
	...
	case VR_SENSOR_THREAD_ID:
		...
		break;
	case MB_TEMP_SENSOR_THREAD_ID:
		count = ARRAY_SIZE(mb_temp_pldm_sensor_table);
		break;
	default:
		count = -1;
		LOG_ERR("Unknow pldm sensor thread id %d", thread_id);
...
void get_pdr_numeric_sesnor(int thread_id, int sensor_num, PDR_numeric_sensor *numeric_sensor_table)
{
	...
	case MB_TEMP_SENSOR_THREAD_ID:
		memcpy(numeric_sensor_table,
		       &mb_temp_pldm_sensor_table[sensor_num].pdr_numeric_sensor,
		       sizeof(PDR_numeric_sensor));
		break;
	default:
		LOG_ERR("Unknow pldm sensor thread id %d", thread_id);

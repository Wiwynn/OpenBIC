...
static int ddr5_spd_read_bytes(uint8_t i2c_bus, uint8_t spd_addr, uint8_t *out)
{
    ...
    typedef struct { uint16_t raw_off; uint16_t comp_off; uint16_t len; } seg_t;
	...
	while (1) {
		...
		for (dimm_id = 0; dimm_id < DIMM_ID_MAX; dimm_id++) {
			...
			if (!dimm_data[dimm_id].is_spd_raw_ready) {
				const uint8_t spd_addr = spd_i3c_addr_list[dimm_id % (DIMM_ID_MAX / 2)];
				...
				if (i2c_master_write(&msg, 1) == 0) {
					if (ddr5_spd_read_bytes(I2C_BUS13, spd_addr, out) == 0) {
						...
						LOG_INF("DIMM %d: SPD compact cache ready (0x50 bytes)", dimm_id);
					} else {
						LOG_DBG("DIMM %d: SPD compact cache read failed", dimm_id);
					}
				} else {
...
void set_spd_raw_ready(uint8_t dimm_id, bool ready)
{
    if (dimm_id < DIMM_ID_MAX)
        dimm_data[dimm_id].is_spd_raw_ready = ready;
}
...
int init_dimm_prsnt_status()
{
	...
	for (uint8_t dimm_id = 0; dimm_id < DIMM_ID_MAX; dimm_id++) {
		...
		if (ret != 0) {
			...
			dimm_data[dimm_id].spd_raw_data[SPD_OEM_STATUS]  &= ~SPD_OEM_STATUS_SPD_READY; 

...
void OEM_1S_SET_POSTCODE_FILTER(ipmi_msg *msg)
{
	...
	set_postcode_filter_enable(val ? true : false);

	LOG_INF("OEM_1S_SET_POSTCODE_FILTER: %s", val ? "enabled" : "disabled");
...
void OEM_1S_GET_POSTCODE_FILTER(ipmi_msg *msg)
{
	...
	bool enabled = get_postcode_filter_enable();
...
void OEM_1S_GET_FILTERED_AMD_POST_CODE(ipmi_msg *msg)
{
	...
	uint8_t count = copy_filtered_amd_postcodes(codes, MAX_FILTERED_AMD_POSTCODES);

	/*
 * Return format:
 *   data[0] = count (actual number of records, max 5)
 *   Following bytes contain records in 4-byte chunks, little-endian format:
 *     data[1..4]   = Record #1 (most recent)
 *     data[5..8]   = Record #2
 *     data[9..12]  = Record #3
 *     ...
 ...
 */
	...
	LOG_INF("OEM_1S_GET_FILTERED_AMD_POST_CODE: count=%d", count);
	return;
}
...
void OEM_1S_GET_POSTCODE_FILTER(ipmi_msg *msg)
{
	...
	LOG_INF("OEM_1S_GET_POSTCODE_FILTER: req data_len=%d", msg->data_len);

	if (msg->data_len != 0) {
		LOG_WRN("OEM_1S_GET_POSTCODE_FILTER invalid length %d", msg->data_len);
		msg->completion_code = CC_INVALID_LENGTH;
		msg->data_len = 0;
		return;
	}
	...
	bool enabled = get_postcode_filter_enable();
	LOG_INF("OEM_1S_GET_POSTCODE_FILTER: enabled=%d", enabled);

	msg->data[0] = enabled ? 1 : 0;
	msg->data_len = 1;
	msg->completion_code = CC_SUCCESS;

	LOG_INF("OEM_1S_GET_POSTCODE_FILTER: resp data[0]=0x%02x len=%d", msg->data[0],
		msg->data_len);
	return;
}

...
void ISR_SOC_THMALTRIP()
{
	common_addsel_msg_t sel_msg;
	if (gpio_get(RST_PLTRST_BMC_N) == GPIO_HIGH) {
		if (gpio_get(FM_CPU_THERMTRIP_LVT3_N) ==
...
void ISR_RMCA()
{
	if ((gpio_get(RST_PLTRST_BMC_N) == GPIO_HIGH) || (gpio_get(PWRGD_CPU_LVC3_R) == GPIO_HIGH)) {
		common_addsel_msg_t sel_msg;
		sel_msg.InF_target = BMC_IPMB;
		sel_msg.sensor_type = IPMI_SENSOR_TYPE_PROCESSOR;
		sel_msg.event_type = IPMI_EVENT_TYPE_SENSOR_SPEC;
...
static void SMI_handler(struct k_work *work)
{
	if (gpio_get(IRQ_SMI_ACTIVE_BMC_N) == GPIO_LOW) {
		...
		ret = common_add_sel_evt_record(&sel_msg);
		if (!ret) {
			printf("Fail to assert SMI event log.\n");
		}
	...
	if (gpio_get(RST_PLTRST_BMC_N) == GPIO_HIGH) {
		if (gpio_get(IRQ_SMI_ACTIVE_BMC_N) == GPIO_LOW) {
			...
			if (is_smi_assert == true) {
				...
				ret = common_add_sel_evt_record(&sel_msg);
				if (!ret) {
					printf("Fail to deassert SMI event log.\n");
				}

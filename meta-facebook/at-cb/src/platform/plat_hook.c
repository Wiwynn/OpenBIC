...
adm1272_init_arg adm1272_init_args[] = {
	[0] = { .is_init = false, .pwr_monitor_cfg = { 0x3F35 }, .is_need_set_pwr_cfg = false },
	[0] = { .is_init = false, .pwr_monitor_cfg = { 0x3F35 }, .is_need_set_pwr_cfg = false },
};
...
ina233_init_arg ina233_aux_init_args[] = {
	[0] = { .is_init = false }, [1] = { .is_init = false },	 [2] = { .is_init = false },
	[3] = { .is_init = false }, [4] = { .is_init = false },	 [5] = { .is_init = false },
	[6] = { .is_init = false }, [7] = { .is_init = false },	 [8] = { .is_init = false },
	[9] = { .is_init = false }, [10] = { .is_init = false }, [11] = { .is_init = false },
...
bool pre_ina233_read(uint8_t sensor_num, void *args)
{
	...
	if (init_arg->is_init != true) {
		...
		msg.tx_len = 3;
		// Current_lsb = 0.001 , r_shunt = 0.001
		// Calibration formula = (0.00512 / (current_lsb * r_shunt)) = 5120(dec) = 0x1400(hex)
		msg.data[0] = INA233_CALIBRATION_OFFSET;
		msg.data[1] = 0x00; // Calibration value
		msg.data[2] = 0x14; // Calibration value
	...
	if (mutex_status != 0) {
		printf("[%s] mutex lock fail, status: %d\n", __func__, mutex_status);
		return false;
	}
...
bool post_ina233_read(uint8_t sensor_num, void *args, int *reading)
{
	...
	if (unlock_status != 0) {
		printf("[%s] unlock fail, status: %d\n", __func__, unlock_status);
		return false;
	}

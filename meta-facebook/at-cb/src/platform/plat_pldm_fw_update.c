...
static uint8_t pldm_atm_apply_check();
...
pldm_fw_update_info_t PLDMUPDATE_FW_CONFIG_TABLE[] = {
	...
	},
	{
		...
		.inf = COMP_UPDATE_VIA_I2C,
		.self_apply_check_func = pldm_atm_apply_check,
		.activate_method = COMP_ACT_AC_PWR_CYCLE,
		.self_act_func = NULL,
		.get_fw_version_fn = NULL,
	},
...
static uint8_t pldm_pre_atm_update(void *fw_update_param)
{
	...
	pldm_fw_update_param_t *p = (pldm_fw_update_param_t *)fw_update_param;
	sensor_cfg *cfg = &sensor_config[sensor_config_index_map[SENSOR_NUM_TEMP_ACCL_1_FREYA_1]];
	accl_card_info *card_info_args = (accl_card_info *)cfg->pre_sensor_read_args;

	if (is_time_to_poll_card_sensor(card_info_args->card_id) != true) {
		LOG_ERR("Artemis module power status not ready, card id: 0x%x",
			card_info_args->card_id);
		return PLDM_FW_UPDATE_ERROR;
	}

	mux_config accl_mux = { 0 };
	if (get_accl_mux_config(card_info_args->card_id, &accl_mux) != true) {
		LOG_ERR("Fail to get ACCL card mux config, card id: 0x%x", card_info_args->card_id);
		return PLDM_FW_UPDATE_ERROR;
	}

	if (set_mux_channel(accl_mux, MUTEX_LOCK_ENABLE) == false) {
		LOG_ERR("ACCL switch card mux fail, card id: 0x%x", card_info_args->card_id);
		return PLDM_FW_UPDATE_ERROR;
	}
	...
	p->bus = I2C_BUS8;
	p->addr = ACCL_ARTEMIS_MODULE_1_ADDR;
...
static uint8_t pldm_post_atm_update(void *fw_update_param)
{
	...
	/* Start sensor polling at the last package */
	if (p->next_len == 0) {
		enable_sensor_poll();
	}

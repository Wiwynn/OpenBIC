...
#define WAIT_QSPI_FIRMWARE_UPDATE_COMPLETE_TIMEOUT_S 10
#define WAIT_PSOC_FIRMWARE_UPDATE_COMPLETE_TIMEOUT_S 40
...
wait_fw_update_status_info atm_wait_fw_info = { .is_init = false, .status = EXEC_STATUS_DEFAULT, .result = EXEC_RESULT_DEFAULT };
		  ...
		  uint32_t image_size, bool is_end_package)
{
	...
	if (is_end_package) {
		...
		ret = sb_cmd_fw_update_bit_operation(bus, addr, BIT_GET,
						     SB_CMD_FW_UPDATE_IN_PROGRESS_BIT);
		if (ret <= 0) {
			if (ret == 0) {
				LOG_ERR("Firmware not report in progress, bus: 0x%x, addr: 0x%x",
					bus, addr);
			} else {
				LOG_ERR("Check update in progress fail, bus: 0x%x, addr: 0x%x", bus,
					addr);
			}
			ret = -1;
			goto exit;
		...
		ret = sb_cmd_fw_update_bit_operation(bus, addr, BIT_CLEAR,
						     SB_CMD_FW_UPDATE_TRANSFER_DONE_BIT);
		if (ret != 0) {
...
exit:
	...
	if (sb_cmd_fw_update_bit_operation(bus, addr, BIT_CLEAR,

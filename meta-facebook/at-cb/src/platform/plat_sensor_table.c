...
sensor_cfg *get_accl_sensor_cfg_info(uint8_t card_id, uint8_t *cfg_count)
{
	...
	switch (card_id) {
	case PCIE_CARD_1:
		cfg = plat_accl1_sensor_config;
		break;
	case PCIE_CARD_2:
		cfg = plat_accl2_sensor_config;
		break;
	case PCIE_CARD_3:
		cfg = plat_accl3_sensor_config;
		break;
	case PCIE_CARD_4:
		cfg = plat_accl4_sensor_config;
		break;
	case PCIE_CARD_5:
		cfg = plat_accl5_sensor_config;
		break;
	case PCIE_CARD_6:
		cfg = plat_accl6_sensor_config;
		break;
	case PCIE_CARD_7:
		cfg = plat_accl7_sensor_config;
		break;
	case PCIE_CARD_8:
		cfg = plat_accl8_sensor_config;
		break;
	case PCIE_CARD_9:
		cfg = plat_accl9_sensor_config;
		break;
	case PCIE_CARD_10:
		cfg = plat_accl10_sensor_config;
		break;
	case PCIE_CARD_11:
		cfg = plat_accl12_sensor_config;
		break;
	case PCIE_CARD_12:
		cfg = plat_accl12_sensor_config;
		break;
...
bool is_pcie_device_access(uint8_t card_id)
{
	...
	if (asic_card_info[card_id].card_status == ASIC_CARD_PRESENT) {
		...
		for (index = 0; index < cfg_count; ++index) {
			switch (cfg_table[index].target_addr) {
			case ACCL_FREYA_1_ADDR:
				if (asic_card_info[card_id].asic_1_status !=
				    ASIC_CARD_DEVICE_PRESENT) {
					cfg_table[index].cache_status = SENSOR_NOT_PRESENT;
				} else {
					cfg_table[index].cache_status = SENSOR_INIT_STATUS;
				}
				break;

...
{
	LOG_INF("SYS_RST_ASIC1_N_R PWR_ON_RST_ASIC1_N ON");
...
{
	LOG_INF("SYS_RST_ASIC2_N_R PWR_ON_RST_ASIC2_N ON");
...
void enable_powers(int cxl_id, int pwr_stage)
{
	...
	case DIMM_POWER_ON_STAGE_3:
		...
		switch (cxl_id) {
		case CXL_ID_1:
			k_timer_start(&enable_asic1_rst_timer, K_MSEC(PWR_RST_DELAY_MSEC), K_NO_WAIT);
			break;
		case CXL_ID_2:
			k_timer_start(&enable_asic2_rst_timer, K_MSEC(PWR_RST_DELAY_MSEC), K_NO_WAIT);
			break;
		default:

...
const vr_fault_info vr_fault_table[] = {
	...
	//ALT_SMB_BIC_PMIC1
	{ PVDDQ_AB_ASIC1, ADDR_IOE1, IOE_P03, PWRGD_PVDDQ_AB_ASIC1, I2C_BUS8, ADDR_VR_P0V85_ASIC1,
	  0 },
	{ P0V85_ASIC1, ADDR_IOE1, IOE_P03, PWRGD_P0V85_ASIC1, I2C_BUS8, ADDR_VR_P0V85_ASIC1, 1 },
	{ PVDDQ_CD_ASIC1, ADDR_IOE1, IOE_P03, PWRGD_PVDDQ_CD_ASIC1, I2C_BUS8, ADDR_VR_P0V8_ASIC1,
	  0 },
	{ P0V8_ASIC1, ADDR_IOE1, IOE_P03, PWRGD_P0V8_ASIC1, I2C_BUS8, ADDR_VR_P0V8_ASIC1, 1 },

	//ALT_SMB_BIC_PMIC2
	{ PVDDQ_AB_ASIC2, ADDR_IOE1, IOE_P04, PWRGD_PVDDQ_AB_ASIC2, I2C_BUS3, ADDR_VR_P0V85_ASIC2,
	  0 },
	{ P0V85_ASIC2, ADDR_IOE1, IOE_P04, PWRGD_P0V85_ASIC2, I2C_BUS3, ADDR_VR_P0V85_ASIC2, 1 },
	{ PVDDQ_CD_ASIC2, ADDR_IOE1, IOE_P04, PWRGD_PVDDQ_CD_ASIC2, I2C_BUS3, ADDR_VR_P0V8_ASIC2,
	  0 },
...
const uint8_t vr_reg_list[][8] = {
	...
	// MPS, MP2971 doesn't support PMBUS_STATUS_MFR_SPECIFIC, 80h is DRMOS_FAULT
	{ PMBUS_STATUS_WORD, PMBUS_STATUS_BYTE, PMBUS_STATUS_VOUT, PMBUS_STATUS_IOUT,
...
void plat_isr_clear_vr_fault(uint8_t vr_addr, uint8_t vr_bus, uint8_t page)
{
	...
	LOG_INF("Clear faults bit success, bus: 0x%x, addr: 0x%x", msg.bus, msg.target_addr);
}
...
void process_pmbus_vr_event_handler(struct k_work *work_item)
{
	...
	for (int i = 0; i < ARRAY_SIZE(vr_fault_table); ++i) {
		...
		LOG_INF("Check VR fault, src: %d, ioe_reg_idx: %d, ioe_bit: %d",
			vr_fault_table[i].vr_source, vr_fault_table[i].ioe_reg_idx,
			vr_fault_table[i].ioe_bit);
		...
		if (i2c_master_write(&msg, retry)) {
			LOG_ERR("Set page failed, bus: 0x%x, addr: 0x%x, page: %d",
				vr_fault_table[i].vr_i2c_bus, vr_fault_table[i].vr_addr,
				vr_fault_table[i].vr_page);
			continue;
		}
		...
		for (int curr_reg = 0; curr_reg < vr_reg_list_len; curr_reg++) {
			...
			if (msg.data[0] == PMBUS_DRMOS_FAULT) {
				msg.rx_len = 2;
				//for MPS MP2971 SPEC read PMBUS_DRMOS_FAULT at page 0
				// set page to page 0 if current power rail is 2
				if (vr_fault_table[i].vr_page == PMBUS_PAGE_1) {
					uint8_t retry = 5;
					msg.tx_len = 2;
					msg.data[0] = PMBUS_PAGE;
...
void ISR_P3V3_E1S_PWR_CHANGE()
{
	...
	if (gpio_get(PWRGD_P3V3_E1S_0_R) == GPIO_LOW) {
		LOG_ERR("PWRGD_P3V3_E1S_0_R is LOW, trigger VR event work handler");
...
void ISR_P12V_E1S_PWR_CHANGE()
{
	...
	if (gpio_get(PWRGD_P12V_E1S_0_R) == GPIO_LOW) {
		LOG_ERR("PWRGD_P12V_E1S_0_R is LOW, trigger VR event work handler");
		k_work_schedule_for_queue(
...
void ISR_SET_CXL_LED_AND_CHECK_PWR_STATUS()
{
	...
	//vr_fault_check
	// If PWRGD_PVTT_CD_ASIC1 is LOW, it means the power is not good.
	// Trigger the VR event work handler to send SEL message.
	if (gpio_get(PWRGD_PVTT_CD_ASIC1) == GPIO_LOW) {
		LOG_ERR("PWRGD_PVTT_CD_ASIC1 is LOW, trigger VR event work handler");
		k_work_schedule_for_queue(
	...
	//vr_fault_check
	// If PWRGD_PVTT_CD_ASIC2 is LOW, it means the power is not good.
	// Trigger the VR event work handler to send SEL message
	if (gpio_get(PWRGD_PVTT_CD_ASIC2) == GPIO_LOW) {
		LOG_ERR("PWRGD_PVTT_CD_ASIC2 is LOW, trigger VR event work handler");
		k_work_schedule_for_queue(
...
void ISR_IOE1_INT_VR_FAULT()
{
	...
	LOG_INF("IOE1_INT_R_N is LOW, trigger VR event work handler");

	if (get_ioe_value(ADDR_IOE2, TCA9555_OUTPUT_PORT_REG_0, &ioe_reg_value) == 0) {
		if ((GETBIT(ioe_reg_value, IOE_P00) == 0) || // SEL_SMB_HOST_MUX_PMIC1_IN_R
		    (GETBIT(ioe_reg_value, IOE_P02) == 0)) // SEL_SMB_HOST_MUX_PMIC2_IN_R
		{
			k_work_schedule_for_queue(
...
{
	LOG_INF("PWRGD_PVTT_AB_ASIC1 is LOW, trigger VR event work handler");
...
{
	LOG_INF("PWRGD_PVTT_AB_ASIC2 is LOW, trigger VR event work handler");
	k_work_schedule_for_queue(&plat_work_q,
...
{
	LOG_INF("PWRGD_PVPP_AB_ASIC1 is LOW, trigger VR event work handler");
	k_work_schedule_for_queue(&plat_work_q,
...
{
	LOG_INF("PWRGD_PVPP_AB_ASIC2 is LOW, trigger VR event work handler");
	k_work_schedule_for_queue(&plat_work_q,
...
{
	LOG_INF("PWRGD_PVPP_CD_ASIC1 is LOW, trigger VR event work handler");
	k_work_schedule_for_queue(&plat_work_q,
...
{
	LOG_INF("PWRGD_PVPP_CD_ASIC2 is LOW, trigger VR event work handler");
	k_work_schedule_for_queue(&plat_work_q,

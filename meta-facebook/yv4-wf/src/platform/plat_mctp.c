...
static void set_dev_endpoint(void)
{
	...
	for (int attempt = 0; attempt < 60; attempt++) {
		...
		if (gpio_get(FM_POWER_EN_R) == POWER_OFF) {
			LOG_INF("DC power off, leave set CXL EID");
			return;
		}
		...
		for (uint8_t i = 0; i < ARRAY_SIZE(plat_mctp_route_tbl); i++) {
			...
			if (p->bus == I2C_BUS_CXL1 &&
			    (!get_cxl_ready_status(CXL_ID_1) || set_eid[CXL_ID_1]))
				continue;

			if (p->bus == I2C_BUS_CXL2 &&
			    (!get_cxl_ready_status(CXL_ID_2) || set_eid[CXL_ID_2]))
				continue;

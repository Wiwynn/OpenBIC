...
bool pal_is_need_mctp_interval(mctp *mctp_inst, uint8_t dest_ep)
{
	switch (mctp_inst->medium_type) {
	case MCTP_MEDIUM_TYPE_SMBUS: {
		mctp_smbus_conf *smbus_conf = (mctp_smbus_conf *)&mctp_inst->medium_conf;
		if ((smbus_conf->bus == I2C_BUS_CXL1) && (plat_get_cxl_eid(CXL_ID_1) == dest_ep)) {
			return true;
		}
		if ((smbus_conf->bus == I2C_BUS_CXL2) && (plat_get_cxl_eid(CXL_ID_2) == dest_ep)) {
			return true;
		}
		break;
	}
...
int pal_get_mctp_interval_ms(uint8_t dest_ep)
{
	if ((dest_ep == plat_get_cxl_eid(CXL_ID_1)) || (dest_ep == plat_get_cxl_eid(CXL_ID_2))) {
		return 1000; // 1s
	}
...
{
	LOG_INF("=====%s", __func__);
	LOG_HEXDUMP_INF(data_buf, 7, "get data");
	uint8_t cxl1_eid = plat_get_cxl_eid(CXL_ID_1);
	uint8_t	cxl2_eid = plat_get_cxl_eid(CXL_ID_2);

	// source is BMC and dest is CXL
	if ((data_buf[2] == 0x8) && ((data_buf[1] == cxl1_eid) || (data_buf[1] == cxl2_eid))) {
		...
		if ((msg_type == MCTP_MSG_TYPE_CCI) && (data_buf[6] == 0x02) && (data_buf[7] == 0x00)) {
					 ...
					 mctp_ext_params ext_params)
{
	...
	case MCTP_MSG_TYPE_CTRL: {
		...
		if (ctrl_hdr->cmd == MCTP_CTRL_CMD_GET_ENDPOINT_ID) {
			...
			memcpy(resp_buf, buf, sizeof(*ctrl_hdr) + 4);
			struct _get_eid_resp *p = (struct _get_eid_resp *) (resp_buf + sizeof(*ctrl_hdr) + 4);
			...
			resp_len = sizeof(*ctrl_hdr) + sizeof(*p) + 4;
			
			resp_buf[1] = buf[2];
			resp_buf[2] = cxl_eid;
			resp_buf[3] = CLEARBIT(buf[3], 3);

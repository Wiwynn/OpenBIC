...
uint8_t _1ou_m2_mapping_table[4] = { 4, 3, 2, 1 }; //Jeff : 理解定義這個做甚麼用
uint8_t _1ou_m2_name_mapping_table[4] = { //Jeff : 理解定義這個做甚麼用
	0x3A,
	0x3C,
	0x3E,
	0x37,
...
void send_gpio_interrupt(uint8_t gpio_num)
{
	...
	status = ipmb_read(
		&msg, IPMB_inf_index_map[msg.InF_target]); //Jeff : IPMB_inf_index_map尋找interface
...
/* Jeff : pmic 沒有implement
K_WORK_DELAYABLE_DEFINE(read_pmic_critical_work, read_pmic_error_via_i3c);*/

void ISR_DC_ON()
{
	set_DC_status(PWRGD_CPU_LVC3); //設定DC status 狀態

	if (get_DC_status() == true) {
		k_work_schedule(&set_DC_on_5s_work, K_SECONDS(DC_ON_5_SECOND)); //等待五秒之後把DC on flag 舉起

		if (k_work_cancel_delayable(&set_DC_off_10s_work) != 0) {
		//因為已經dc on 了，如果set_DC_off_10s_work在workqueue的情況下，要取消把dc off flag 舉起
			LOG_ERR("Cancel set dc off delay work fail");
		}
		set_DC_off_delayed_status(); //設定dc off flag
	} else {
		...
		/* Jeff : pmic 沒有implement
		k_work_schedule(&read_pmic_critical_work,
				K_MSEC(READ_PMIC_CRITICAL_ERROR_MS));*/
...
void ISR_PWRGD_CPU()
{
	...
	if (gpio_get(PWRGD_CPU_LVC3) == GPIO_HIGH) {
		...
		k_work_schedule(
			&PROC_FAIL_work,
			K_SECONDS(
				PROC_FAIL_START_DELAY_SECOND)); //這個function 在檢驗kcs 和 postcode是否get到，沒有的話代表cpu沒有開成功
	} else {
		abort_snoop_thread();
...
void ISR_CATERR() // Jeff : 不懂意義，問bios，跟CPU error 有關
{
...
void ISR_PLTRST()
{
	send_gpio_interrupt(RST_PLTRST_BUF_N); // Jeff : 問cpld，(這個PIN HIGH LOW的時候會做什麼事情)
}
...
void ISR_HSC_THROTTLE() //throttle的問題問lora
{
	common_addsel_msg_t sel_msg;
	static bool is_hsc_throttle_assert = false; // Flag for filter out fake alert
	if (gpio_get(PWRGD_AUX_PWRGD_BMC_LVC3) == GPIO_HIGH) { //Jeff : 為什麼要判斷這兩根pin
		if (gpio_get(PWRGD_CPU_LVC3) == GPIO_HIGH) {
			return;
		} else {
			if ((gpio_get(IRQ_SML1_PMBUS_ALERT_N) ==
			     GPIO_HIGH) && //Jeff : 了解這跟pin high low 產生的狀況
...
static void mb_throttle_handler(struct k_work *work)
{
	...
	if (((gpio_get(FAST_PROCHOT_N) == LOW_ACTIVE) && (is_mb_throttle_assert == false)) ||
		...
		sel_msg.InF_target = BMC_IPMB;
		sel_msg.sensor_type = IPMI_OEM_SENSOR_TYPE_SYS_STA; //Jeff : 理解STA在做啥，好像得不到答案
		sel_msg.sensor_number = SENSOR_NUM_SYSTEM_STATUS;
...
void ISR_MB_THROTTLE() //Jeff : 理解throttle 是在幹嘛
{
...
{
	common_addsel_msg_t sel_msg;
	if (gpio_get(RST_PLTRST_SYNC_LVC3_N) ==
	    GPIO_HIGH) { //Jeff implment : 確認RST_PLTRST_SYNC_LVC3_N 是不是CL 的gpioC7
...
void ISR_SYS_THROTTLE() //Jeff : 理解throttle 是在幹嘛
{
	common_addsel_msg_t sel_msg;
...
void ISR_HSC_OC() //OC : over current
{
	common_addsel_msg_t sel_msg;
	if (gpio_get(PWRGD_AUX_PWRGD_BMC_LVC3) == GPIO_HIGH) {
...
void ISR_CPU_MEMHOT()
{
	...
	if ((gpio_get(RST_PLTRST_SYNC_LVC3_N) == GPIO_HIGH) &&
		...
		if (gpio_get(H_CPU_MEMHOT_OUT_LVC3_N) == GPIO_HIGH) {
			sel_msg.event_type =
				IPMI_OEM_EVENT_TYPE_DEASSERT; //Jeff : 這兩個event type 是甚麼
		} else {
			sel_msg.event_type = IPMI_EVENT_TYPE_SENSOR_SPECIFIC;
		}
...
void ISR_CPUVR_HOT()
{
	...
	if ((gpio_get(RST_PLTRST_SYNC_LVC3_N) == GPIO_HIGH) &&
	    (gpio_get(PWRGD_CPU_LVC3) == GPIO_HIGH)) {
		if (gpio_get(IRQ_PVCCIN_CPU0_VRHOT_N) ==
		    GPIO_HIGH) { //Jeff implement : excel 有列IRQ_PVCCIN_CPU0_VRHOT_N, IRQ_PVCCINF_CPU0_VRHOT_N兩項，這個是判斷其中一個舉起來
			sel_msg.event_type = IPMI_OEM_EVENT_TYPE_DEASSERT;
		} else {
			sel_msg.event_type = IPMI_EVENT_TYPE_SENSOR_SPECIFIC;
		}
...
void ISR_CPU_VPP_INT() // Jeff : 參考VPP powerpoint
{
	if (gpio_get(PWRGD_CPU_LVC3) == POWER_ON) {
		...
		/*post complete pin hasn't been ready yet*/
		...
		for (device_id = 0; device_id < MAX_1OU_M2_COUNT; device_id++) {
			...
			if (gpio_get(PWRGD_CPU_LVC3) == LOW_INACTIVE) {
				memset(&sel_msg, 0, sizeof(common_addsel_msg_t));
				sel_msg.InF_target = BMC_IPMB;
				sel_msg.sensor_type = IPMI_OEM_SENSOR_TYPE_SYS_STA;

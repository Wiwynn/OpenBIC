...
static inline int jtag_freq_get(const struct device *dev, uint32_t *freq)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&freq,
						 K_SYSCALL_JTAG_FREQ_GET);
	}
#endif
	compiler_barrier();
...
static inline int jtag_tap_get(const struct device *dev, enum tap_state *state)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&state,
						 K_SYSCALL_JTAG_TAP_GET);
	}
#endif
	compiler_barrier();
...
static inline int jtag_tck_run(const struct device *dev, uint32_t run_count)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&run_count,
						 K_SYSCALL_JTAG_TCK_RUN);
	}
#endif
	compiler_barrier();
...
extern int z_impl_jtag_ir_scan(const struct device *dev, int num_bits, const uint8_t *out_value,
			       uint8_t *in_value, enum tap_state state);
static inline int jtag_ir_scan(const struct device *dev, int num_bits, const uint8_t *out_value,
			       uint8_t *in_value, enum tap_state state)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke5(*(uintptr_t *)&dev, *(uintptr_t *)&num_bits,
						 *(uintptr_t *)&out_value, *(uintptr_t *)&in_value,
						 *(uintptr_t *)&state, K_SYSCALL_JTAG_IR_SCAN);
	}
#endif
	compiler_barrier();
...
extern int z_impl_jtag_dr_scan(const struct device *dev, int num_bits, const uint8_t *out_value,
			       uint8_t *in_value, enum tap_state state);
static inline int jtag_dr_scan(const struct device *dev, int num_bits, const uint8_t *out_value,
			       uint8_t *in_value, enum tap_state state)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		...
		return (int)arch_syscall_invoke5(*(uintptr_t *)&dev, *(uintptr_t *)&num_bits,
						 *(uintptr_t *)&out_value, *(uintptr_t *)&in_value,
						 *(uintptr_t *)&state, K_SYSCALL_JTAG_DR_SCAN);
	}
#endif
	compiler_barrier();
...
static inline int jtag_sw_xfer(const struct device *dev, enum jtag_pin pin, uint8_t value)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&pin,
						 *(uintptr_t *)&value, K_SYSCALL_JTAG_SW_XFER);
	}
#endif
	compiler_barrier();
...
static inline int jtag_tdo_get(const struct device *dev, uint8_t *value)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&value,
						 K_SYSCALL_JTAG_TDO_GET);
	}
#endif
	compiler_barrier();

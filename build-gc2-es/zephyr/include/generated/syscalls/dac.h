...
extern int z_impl_dac_channel_setup(const struct device *dev,
				    const struct dac_channel_cfg *channel_cfg);
static inline int dac_channel_setup(const struct device *dev,
				    const struct dac_channel_cfg *channel_cfg)
{
...
static inline int dac_write_value(const struct device *dev, uint8_t channel, uint32_t value)
{
	...
	if (z_syscall_trap()) {
		...
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&channel,
						 *(uintptr_t *)&value, K_SYSCALL_DAC_WRITE_VALUE);
	}
#endif
	compiler_barrier();

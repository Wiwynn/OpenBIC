...
extern int z_impl_espi_saf_set_protection_regions(const struct device *dev,
						  const struct espi_saf_protection *pr);
static inline int espi_saf_set_protection_regions(const struct device *dev,
						  const struct espi_saf_protection *pr)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&pr,
...
static inline bool espi_saf_get_channel_status(const struct device *dev)
{
	...
	if (z_syscall_trap()) {
		...
		return (bool)arch_syscall_invoke1(*(uintptr_t *)&dev,
						  K_SYSCALL_ESPI_SAF_GET_CHANNEL_STATUS);
	}
#endif
	compiler_barrier();
...
static inline int espi_saf_flash_erase(const struct device *dev, struct espi_saf_packet *pckt)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&pckt,
						 K_SYSCALL_ESPI_SAF_FLASH_ERASE);
	}
#endif
	compiler_barrier();

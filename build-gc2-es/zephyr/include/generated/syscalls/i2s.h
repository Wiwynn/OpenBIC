...
extern int z_impl_i2s_configure(const struct device *dev, enum i2s_dir dir,
				...
				const struct i2s_config *cfg)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&dir,
						 *(uintptr_t *)&cfg, K_SYSCALL_I2S_CONFIGURE);
	}
#endif
	compiler_barrier();
...
static inline int i2s_buf_read(const struct device *dev, void *buf, size_t *size)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&buf,
						 *(uintptr_t *)&size, K_SYSCALL_I2S_BUF_READ);
	}
#endif
	compiler_barrier();
...
static inline int i2s_buf_write(const struct device *dev, void *buf, size_t size)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&buf,
						 *(uintptr_t *)&size, K_SYSCALL_I2S_BUF_WRITE);
	}
#endif
	compiler_barrier();
...
static inline int i2s_trigger(const struct device *dev, enum i2s_dir dir, enum i2s_trigger_cmd cmd)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&dir,
						 *(uintptr_t *)&cmd, K_SYSCALL_I2S_TRIGGER);
	}
#endif
	compiler_barrier();

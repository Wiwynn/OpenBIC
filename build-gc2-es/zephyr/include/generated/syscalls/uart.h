...
extern int z_impl_uart_tx(const struct device *dev, const uint8_t *buf, size_t len,
			  int32_t timeout);
static inline int uart_tx(const struct device *dev, const uint8_t *buf, size_t len, int32_t timeout)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke4(*(uintptr_t *)&dev, *(uintptr_t *)&buf,
						 *(uintptr_t *)&len, *(uintptr_t *)&timeout,
						 K_SYSCALL_UART_TX);
	}
#endif
	compiler_barrier();
...
extern int z_impl_uart_rx_enable(const struct device *dev, uint8_t *buf, size_t len,
				 int32_t timeout);
...
static inline int uart_poll_in(const struct device *dev, unsigned char *p_char)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&p_char,
...
static inline void uart_poll_out(const struct device *dev, unsigned char out_char)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&out_char,
...
static inline int uart_line_ctrl_set(const struct device *dev, uint32_t ctrl, uint32_t val)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&ctrl,
						 *(uintptr_t *)&val, K_SYSCALL_UART_LINE_CTRL_SET);
	}
#endif
	compiler_barrier();
...
static inline int uart_line_ctrl_get(const struct device *dev, uint32_t ctrl, uint32_t *val)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&ctrl,
						 *(uintptr_t *)&val, K_SYSCALL_UART_LINE_CTRL_GET);
	}
#endif
	compiler_barrier();
...
static inline int uart_drv_cmd(const struct device *dev, uint32_t cmd, uint32_t p)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&cmd,
						 *(uintptr_t *)&p, K_SYSCALL_UART_DRV_CMD);
	}
#endif
	compiler_barrier();

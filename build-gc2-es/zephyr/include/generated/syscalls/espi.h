...
static inline int espi_config(const struct device *dev, struct espi_cfg *cfg)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&cfg,
...
static inline bool espi_get_channel_status(const struct device *dev, enum espi_channel ch)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (bool)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&ch,
						  K_SYSCALL_ESPI_GET_CHANNEL_STATUS);
	}
#endif
	compiler_barrier();
...
static inline int espi_read_request(const struct device *dev, struct espi_request_packet *req)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&req,
						 K_SYSCALL_ESPI_READ_REQUEST);
	}
#endif
	compiler_barrier();
...
extern int z_impl_espi_read_lpc_request(const struct device *dev, enum lpc_peripheral_opcode op,
					uint32_t *data);
static inline int espi_read_lpc_request(const struct device *dev, enum lpc_peripheral_opcode op,
					uint32_t *data)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&op,
						 *(uintptr_t *)&data,
...
extern int z_impl_espi_write_lpc_request(const struct device *dev, enum lpc_peripheral_opcode op,
					 uint32_t *data);
static inline int espi_write_lpc_request(const struct device *dev, enum lpc_peripheral_opcode op,
					 uint32_t *data)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
...
extern int z_impl_espi_send_vwire(const struct device *dev, enum espi_vwire_signal signal,
				  uint8_t level);
static inline int espi_send_vwire(const struct device *dev, enum espi_vwire_signal signal,
				  uint8_t level)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&signal,
						 *(uintptr_t *)&level, K_SYSCALL_ESPI_SEND_VWIRE);
	}
#endif
	compiler_barrier();
...
extern int z_impl_espi_receive_vwire(const struct device *dev, enum espi_vwire_signal signal,
				     uint8_t *level);
static inline int espi_receive_vwire(const struct device *dev, enum espi_vwire_signal signal,
				     uint8_t *level)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
...
static inline int espi_send_oob(const struct device *dev, struct espi_oob_packet *pckt)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&pckt,
						 K_SYSCALL_ESPI_SEND_OOB);
	}
#endif
	compiler_barrier();

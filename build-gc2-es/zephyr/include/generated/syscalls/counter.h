...
static inline bool counter_is_counting_up(const struct device *dev)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (bool)arch_syscall_invoke1(*(uintptr_t *)&dev,
...
static inline uint8_t counter_get_num_of_channels(const struct device *dev)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (uint8_t)arch_syscall_invoke1(*(uintptr_t *)&dev,
						     K_SYSCALL_COUNTER_GET_NUM_OF_CHANNELS);
	}
#endif
	compiler_barrier();
...
static inline uint32_t counter_get_frequency(const struct device *dev)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (uint32_t)arch_syscall_invoke1(*(uintptr_t *)&dev,
						      K_SYSCALL_COUNTER_GET_FREQUENCY);
	}
#endif
	compiler_barrier();
...
static inline uint32_t counter_us_to_ticks(const struct device *dev, uint64_t us)
{
	...
	if (z_syscall_trap()) {
		...
		parm0.val = us;
		/* coverity[OVERRUN] */
		return (uint32_t)arch_syscall_invoke3(*(uintptr_t *)&dev, parm0.split.lo,
						      parm0.split.hi,
						      K_SYSCALL_COUNTER_US_TO_TICKS);
	}
#endif
	compiler_barrier();
...
static inline uint64_t counter_ticks_to_us(const struct device *dev, uint32_t ticks)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		(void)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&ticks,
					   (uintptr_t)&ret64, K_SYSCALL_COUNTER_TICKS_TO_US);
...
static inline uint32_t counter_get_max_top_value(const struct device *dev)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (uint32_t)arch_syscall_invoke1(*(uintptr_t *)&dev,
						      K_SYSCALL_COUNTER_GET_MAX_TOP_VALUE);
	}
#endif
	compiler_barrier();
...
static inline int counter_get_value(const struct device *dev, uint32_t *ticks)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&ticks,
						 K_SYSCALL_COUNTER_GET_VALUE);
	}
#endif
	compiler_barrier();
...
extern int z_impl_counter_set_channel_alarm(const struct device *dev, uint8_t chan_id,
					    const struct counter_alarm_cfg *alarm_cfg);
static inline int counter_set_channel_alarm(const struct device *dev, uint8_t chan_id,
					    const struct counter_alarm_cfg *alarm_cfg)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&chan_id,
						 *(uintptr_t *)&alarm_cfg,
...
static inline int counter_cancel_channel_alarm(const struct device *dev, uint8_t chan_id)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&chan_id,
						 K_SYSCALL_COUNTER_CANCEL_CHANNEL_ALARM);
	}
#endif
	compiler_barrier();
...
extern int z_impl_counter_set_top_value(const struct device *dev,
...
static inline int counter_set_top_value(const struct device *dev, const struct counter_top_cfg *cfg)
{
	...
	if (z_syscall_trap()) {
		...
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&cfg,
						 K_SYSCALL_COUNTER_SET_TOP_VALUE);
	}
#endif
	compiler_barrier();
...
static inline int counter_get_pending_int(const struct device *dev)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke1(*(uintptr_t *)&dev,
...
extern int z_impl_counter_set_guard_period(const struct device *dev, uint32_t ticks,
					   uint32_t flags);
static inline int counter_set_guard_period(const struct device *dev, uint32_t ticks, uint32_t flags)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&ticks,
						 *(uintptr_t *)&flags,
...
static inline uint32_t counter_get_guard_period(const struct device *dev, uint32_t flags)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (uint32_t)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&flags,
						      K_SYSCALL_COUNTER_GET_GUARD_PERIOD);
	}
#endif
	compiler_barrier();

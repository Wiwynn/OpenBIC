...
static inline int dma_start(const struct device *dev, uint32_t channel)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&channel,
						 K_SYSCALL_DMA_START);
	}
#endif
	compiler_barrier();
...
static inline int dma_request_channel(const struct device *dev, void *filter_param)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&filter_param,
...
static inline void dma_release_channel(const struct device *dev, uint32_t channel)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&channel,
				     K_SYSCALL_DMA_RELEASE_CHANNEL);
		return;
	}
#endif
	compiler_barrier();

...
extern int z_impl_pwm_pin_set_cycles(const struct device *dev, uint32_t pwm, uint32_t period,
				     uint32_t pulse, pwm_flags_t flags);
static inline int pwm_pin_set_cycles(const struct device *dev, uint32_t pwm, uint32_t period,
				     uint32_t pulse, pwm_flags_t flags)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke5(*(uintptr_t *)&dev, *(uintptr_t *)&pwm,
						 *(uintptr_t *)&period, *(uintptr_t *)&pulse,
						 *(uintptr_t *)&flags,
...
static inline int pwm_pin_enable_capture(const struct device *dev, uint32_t pwm)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&pwm,
						 K_SYSCALL_PWM_PIN_ENABLE_CAPTURE);
	}
#endif
	compiler_barrier();
...
static inline int pwm_pin_disable_capture(const struct device *dev, uint32_t pwm)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&pwm,
						 K_SYSCALL_PWM_PIN_DISABLE_CAPTURE);
	}
#endif
	compiler_barrier();
...
extern int z_impl_pwm_pin_capture_cycles(const struct device *dev, uint32_t pwm, pwm_flags_t flags,
					 uint32_t *period, uint32_t *pulse, k_timeout_t timeout);
static inline int pwm_pin_capture_cycles(const struct device *dev, uint32_t pwm, pwm_flags_t flags,
					 uint32_t *period, uint32_t *pulse, k_timeout_t timeout)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		...
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke6(*(uintptr_t *)&dev, *(uintptr_t *)&pwm,
						 *(uintptr_t *)&flags, *(uintptr_t *)&period,
						 *(uintptr_t *)&pulse, (uintptr_t)&more,
						 K_SYSCALL_PWM_PIN_CAPTURE_CYCLES);
	}
#endif
	compiler_barrier();
...
static inline int pwm_get_cycles_per_sec(const struct device *dev, uint32_t pwm, uint64_t *cycles)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&pwm,
						 *(uintptr_t *)&cycles,

...
static inline int flash_read(const struct device *dev, off_t offset, void *data, size_t len)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke4(*(uintptr_t *)&dev, *(uintptr_t *)&offset,
...
extern int z_impl_flash_reg_read(const struct device *dev, off_t offset, void *data, size_t len);
static inline int flash_reg_read(const struct device *dev, off_t offset, void *data, size_t len)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke4(*(uintptr_t *)&dev, *(uintptr_t *)&offset,
						 *(uintptr_t *)&data, *(uintptr_t *)&len,
						 K_SYSCALL_FLASH_REG_READ);
	}
#endif
	compiler_barrier();
	return z_impl_flash_reg_read(dev, offset, data, len);
}

extern int z_impl_flash_reg_write(const struct device *dev, off_t offset, const void *data,
				  size_t len);
static inline int flash_reg_write(const struct device *dev, off_t offset, const void *data,
				  size_t len)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		...
		return (int)arch_syscall_invoke4(*(uintptr_t *)&dev, *(uintptr_t *)&offset,
						 *(uintptr_t *)&data, *(uintptr_t *)&len,
						 K_SYSCALL_FLASH_REG_WRITE);
	}
#endif
	compiler_barrier();
	return z_impl_flash_reg_write(dev, offset, data, len);
}
...
static inline int flash_erase(const struct device *dev, off_t offset, size_t size)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&offset,
						 *(uintptr_t *)&size, K_SYSCALL_FLASH_ERASE);
	}
#endif
	compiler_barrier();
...
static inline int flash_write_protection_set(const struct device *dev, bool enable)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&enable,
...
extern int z_impl_flash_get_page_info_by_offs(const struct device *dev, off_t offset,
					      struct flash_pages_info *info);
static inline int flash_get_page_info_by_offs(const struct device *dev, off_t offset,
					      struct flash_pages_info *info)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
...
extern int z_impl_flash_get_page_info_by_idx(const struct device *dev, uint32_t page_index,
					     struct flash_pages_info *info);
static inline int flash_get_page_info_by_idx(const struct device *dev, uint32_t page_index,
					     struct flash_pages_info *info)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&page_index,
						 *(uintptr_t *)&info,
...
static inline size_t flash_get_page_count(const struct device *dev)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (size_t)arch_syscall_invoke1(*(uintptr_t *)&dev,
...
static inline int flash_read_jedec_id(const struct device *dev, uint8_t *id)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&id,
						 K_SYSCALL_FLASH_READ_JEDEC_ID);
	}
#endif
	compiler_barrier();
...
extern size_t z_impl_flash_get_flash_size(const struct device *dev);
static inline size_t flash_get_flash_size(const struct device *dev)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (size_t)arch_syscall_invoke1(*(uintptr_t *)&dev,
						    K_SYSCALL_FLASH_GET_FLASH_SIZE);
	}
#endif
	compiler_barrier();
	return z_impl_flash_get_flash_size(dev);
}
...
static inline const struct flash_parameters *flash_get_parameters(const struct device *dev)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (const struct flash_parameters *)arch_syscall_invoke1(
			*(uintptr_t *)&dev, K_SYSCALL_FLASH_GET_PARAMETERS);
	}
#endif
	compiler_barrier();

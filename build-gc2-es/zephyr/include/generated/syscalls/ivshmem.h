...
static inline size_t ivshmem_get_mem(const struct device *dev, uintptr_t *memmap)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (size_t)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&memmap,
...
static inline uint16_t ivshmem_get_vectors(const struct device *dev)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (uint16_t)arch_syscall_invoke1(*(uintptr_t *)&dev,
						      K_SYSCALL_IVSHMEM_GET_VECTORS);
	}
#endif
	compiler_barrier();
...
static inline int ivshmem_int_peer(const struct device *dev, uint32_t peer_id, uint16_t vector)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&peer_id,
						 *(uintptr_t *)&vector, K_SYSCALL_IVSHMEM_INT_PEER);
	}
#endif
	compiler_barrier();
...
extern int z_impl_ivshmem_register_handler(const struct device *dev, struct k_poll_signal *signal,
					   uint16_t vector);
static inline int ivshmem_register_handler(const struct device *dev, struct k_poll_signal *signal,
					   uint16_t vector)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		...
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&signal,
						 *(uintptr_t *)&vector,
						 K_SYSCALL_IVSHMEM_REGISTER_HANDLER);
	}
#endif
	compiler_barrier();

...
extern int z_impl_led_blink(const struct device *dev, uint32_t led, uint32_t delay_on,
			    uint32_t delay_off);
static inline int led_blink(const struct device *dev, uint32_t led, uint32_t delay_on,
			    uint32_t delay_off)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke4(*(uintptr_t *)&dev, *(uintptr_t *)&led,
						 *(uintptr_t *)&delay_on, *(uintptr_t *)&delay_off,
						 K_SYSCALL_LED_BLINK);
	}
#endif
	compiler_barrier();
...
extern int z_impl_led_get_info(const struct device *dev, uint32_t led,
...
static inline int led_get_info(const struct device *dev, uint32_t led, const struct led_info **info)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&led,
						 *(uintptr_t *)&info, K_SYSCALL_LED_GET_INFO);
	}
#endif
	compiler_barrier();
...
extern int z_impl_led_write_channels(const struct device *dev, uint32_t start_channel,
				     uint32_t num_channels, const uint8_t *buf);
static inline int led_write_channels(const struct device *dev, uint32_t start_channel,
				     uint32_t num_channels, const uint8_t *buf)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke4(*(uintptr_t *)&dev, *(uintptr_t *)&start_channel,
						 *(uintptr_t *)&num_channels, *(uintptr_t *)&buf,
						 K_SYSCALL_LED_WRITE_CHANNELS);
	}
#endif
	compiler_barrier();
...
static inline int led_set_channel(const struct device *dev, uint32_t channel, uint8_t value)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke3(*(uintptr_t *)&dev, *(uintptr_t *)&channel,
						 *(uintptr_t *)&value, K_SYSCALL_LED_SET_CHANNEL);
	}
#endif
	compiler_barrier();
...
extern int z_impl_led_set_color(const struct device *dev, uint32_t led, uint8_t num_colors,
				const uint8_t *color);
static inline int led_set_color(const struct device *dev, uint32_t led, uint8_t num_colors,
				const uint8_t *color)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke4(*(uintptr_t *)&dev, *(uintptr_t *)&led,
						 *(uintptr_t *)&num_colors, *(uintptr_t *)&color,
						 K_SYSCALL_LED_SET_COLOR);
	}
#endif
	compiler_barrier();
...
static inline int led_on(const struct device *dev, uint32_t led)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dev, *(uintptr_t *)&led,
						 K_SYSCALL_LED_ON);
	}
#endif
	compiler_barrier();

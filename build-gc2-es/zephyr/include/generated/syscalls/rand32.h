...
static inline void sys_rand_get(void *dst, size_t len)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		arch_syscall_invoke2(*(uintptr_t *)&dst, *(uintptr_t *)&len,
				     K_SYSCALL_SYS_RAND_GET);
		return;
	}
#endif
	compiler_barrier();
...
static inline int sys_csrand_get(void *dst, size_t len)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&dst, *(uintptr_t *)&len,
						 K_SYSCALL_SYS_CSRAND_GET);
	}
#endif
	compiler_barrier();

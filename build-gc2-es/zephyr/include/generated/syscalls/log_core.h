...
static inline void z_log_string_from_user(uint32_t src_level_val, const char *str)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		arch_syscall_invoke2(*(uintptr_t *)&src_level_val, *(uintptr_t *)&str,
				     K_SYSCALL_Z_LOG_STRING_FROM_USER);
		return;
	}
#endif
	compiler_barrier();
...
extern void z_impl_z_log_hexdump_from_user(uint32_t src_level_val, const char *metadata,
					   const uint8_t *data, uint32_t len);
static inline void z_log_hexdump_from_user(uint32_t src_level_val, const char *metadata,
					   const uint8_t *data, uint32_t len)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		arch_syscall_invoke4(*(uintptr_t *)&src_level_val, *(uintptr_t *)&metadata,
				     *(uintptr_t *)&data, *(uintptr_t *)&len,
				     K_SYSCALL_Z_LOG_HEXDUMP_FROM_USER);
		return;
	}
#endif
	compiler_barrier();

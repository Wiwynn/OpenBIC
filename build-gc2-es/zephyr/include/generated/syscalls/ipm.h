...
extern int z_impl_ipm_send(const struct device *ipmdev, int wait, uint32_t id, const void *data,
			   int size);
static inline int ipm_send(const struct device *ipmdev, int wait, uint32_t id, const void *data,
			   int size)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke5(*(uintptr_t *)&ipmdev, *(uintptr_t *)&wait,
						 *(uintptr_t *)&id, *(uintptr_t *)&data,
						 *(uintptr_t *)&size, K_SYSCALL_IPM_SEND);
	}
#endif
	compiler_barrier();
...
static inline int ipm_max_data_size_get(const struct device *ipmdev)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke1(*(uintptr_t *)&ipmdev,
...
static inline uint32_t ipm_max_id_val_get(const struct device *ipmdev)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (uint32_t)arch_syscall_invoke1(*(uintptr_t *)&ipmdev,
...
static inline int ipm_set_enabled(const struct device *ipmdev, int enable)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&ipmdev, *(uintptr_t *)&enable,

...
static inline int z_zephyr_read_stdin(char *buf, int nbytes)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&buf, *(uintptr_t *)&nbytes,
						 K_SYSCALL_Z_ZEPHYR_READ_STDIN);
	}
#endif
	compiler_barrier();
...
static inline int z_zephyr_write_stdout(const void *buf, int nbytes)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&buf, *(uintptr_t *)&nbytes,
						 K_SYSCALL_Z_ZEPHYR_WRITE_STDOUT);
	}
#endif
	compiler_barrier();
...
static inline int zephyr_fputc(int c, FILE *stream)
{
	...
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int)arch_syscall_invoke2(*(uintptr_t *)&c, *(uintptr_t *)&stream,
						 K_SYSCALL_ZEPHYR_FPUTC);
	}
#endif
	compiler_barrier();
...
extern size_t z_impl_zephyr_fwrite(const void *_MLIBC_RESTRICT ptr, size_t size, size_t nitems,
				   FILE *_MLIBC_RESTRICT stream);
static inline size_t zephyr_fwrite(const void *_MLIBC_RESTRICT ptr, size_t size, size_t nitems,
				   FILE *_MLIBC_RESTRICT stream)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (size_t)arch_syscall_invoke4(*(uintptr_t *)&ptr, *(uintptr_t *)&size,
						    *(uintptr_t *)&nitems, *(uintptr_t *)&stream,
						    K_SYSCALL_ZEPHYR_FWRITE);
	}
#endif
	compiler_barrier();
